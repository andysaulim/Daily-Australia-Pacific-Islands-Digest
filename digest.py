...
- calendar_watch: 4-5 entries, nearest first, drawn ONLY from today's articles or the VERIFIED DIPLOMATIC CALENDAR. Each: date (ISO if confirmed, else null), window (a phrase naming a month, like "expected in August" or "late November 2026", else null), event, why_it_matters (1 sentence, on what it changes for Australia, New Zealand or the Pacific), confirmed (boolean).
  Every entry must carry a day or a month. A cadence — "annual", "venue alternates", "roughly twice yearly" — is not a window and the entry is dropped before it prints, so leave it out instead.
  Prefer what this issue actually reported: a sitting week, a court return, a ruling, a ratification deadline, a rate decision, a visit already announced in one of today's articles. A recurring summit a year out is a last resort, not a first choice. Four entries a reader can act on beat five padded with standing fixtures.
...
#
# 64000, not 32000. The first live run hit the 32k ceiling mid-JSON on attempt
# one, which surfaced as "Empty response from the Claude API" and burned a whole
# generation attempt before Opus produced a clean brief on attempt two. Enriched
# summaries made the input, and the output, materially larger than the Korea
# brief this was inherited from. The models support 128k here, and an unused
