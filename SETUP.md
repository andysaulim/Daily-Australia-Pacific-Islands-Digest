...
Leave `WEB_URL` unset and the read-online bar does not render, and with it
goes the **Download PDF** link: both point into the published archive, so
without Pages there is nothing on the end of either. The archive itself is
still written and deployed either way.

The PDF is generated by `pdf_export.py` from the archived HTML, not
re-rendered, so the file a reader downloads is the issue that was sent. It
needs headless Chromium, which the workflow installs in its own
`continue-on-error` step: a browser that will not install costs you the PDF
and nothing else. The email link is dated (`digest_2026-09-02.pdf`) rather
than `latest.pdf`, so opening an old issue's link a week later gives you
that issue rather than whatever shipped this morning.
